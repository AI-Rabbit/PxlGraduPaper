...
\begin{cnabstract}{可重构智能表面；智能反射面；无线中继；大规模多进多出系统；原型系统；现场可编程逻辑门阵列}
	...
	近年来，人们研制出了能够对撞击的无线电波进行特定变换的基于电磁的
可重构结构，它们的工作频段非常广阔，可以覆盖 Sub­6 GHz、毫米波甚至太赫
兹[8] 。下面将详细介绍这一新技术
	近年来，人们研制出了能够对撞击的无线电波进行特定变换的基于电磁的可重构结构，
	可重构智能表面(Reconfigured Intelligent
Surface, RIS)是一种具有大量无源元件的二维结构，近年来被认为是实现节能智能无线通信
的革命性技术之一。在本文中，我们开发了一种新型的高增益低成本RIS，它具有256 个元
件。所提出的RIS 结合了相移和电磁表面辐射的功能，其中PIN 二极管用于实现2 位相移
的波束赋形。这种激进的设计构成了世界上第一个无线通信原型的基础，该原型使用了256
个2 位元RIS。该原型由模块化硬件和灵活的软件组成，包括以下内容：参数设置和数据交
换主机，用于基带和射频信号处理的通用软件无线电外设(USRP)，以及用于信号传输和接
收的RIS。我们的性能评估证实了RIS 在无线通信中的可行性和有效性。我们表明，在2.3
GHz，所提出的RIS 可以实现21.7 dBi 天线增益。在毫米波(mm Wave)频率，即28.5 GHz，
天线增益为19.1 dBi。此外，研究表明，基于RIS 的无线通信原型能够显著降低功耗。
...
时下，第二版的毫米波系统刚刚建立，通信链路刚刚调通，智能超表面在毫米波通信中的应用更加值得我们去探索，本文的主要内容也是围绕毫米波系统展开的


\begin{thankpage}
	
	“未觉池塘春草梦，阶前梧叶已秋声。”宋朝大儒朱子以敏锐细腻的笔触感叹岁月易逝。此情此景我在华工也是深有感触。
