...
打一个生动的比方：这就像老师领着一队小孩去食堂就餐，如果只有一个窗口，他们只能一个一个排队打饭。
而如果食堂有很多窗口，所以虽然他们拍成一长队来食堂，但是几个小孩却可以同时打饭，这大大提高了食堂的运转效率，减少了小孩们的等待时间。
% 在这种配置中，每个触发器是边沿触发。
% 所有触发器都以给定的时钟频率运行。
% 每个输入位在$n$个时钟周期后将移动到第$n$个触发器的输出端，导致并行输出。
如果食堂有$n$个窗口，则$n$个小孩可以同时打饭，代表着$n$位的移位寄存器可以实现$n$位的数据并行输出。
