...
其中一种采用梯度下降方法设计，而另一种是基于分式规划方法。其中的仿真结果表明，与传统的多天线自动对焦中继相比，该系统能够获得高达300\%的能效\cite{huang2019reconfigurable}。
...
假设有一条路径能让电磁波沿着它传播，如果我们能够引入一个相位变化，就可以达到控制波前的目的。
% 沿波传者有一径，引一相移变于波长内，可制波前。
% 过沿着电磁波传播路径，在波长范围内引入突变相移，可以获得控制波前的新自由度。
% 费马原理指出光线在两点A和B之间的轨迹是最小光程的轨迹，即$ \int_{A}^{B} n(\vec{r}) dr $，其中$ n(\vec{r}) $是局部折射率，由此容易推演得到两种介质之间的反射和折射定律。
我们可能会想起高中时代学习过的费马原理，光走过两点之间的路程可以表述为$ \int_{A}^{B} n(\vec{r}) dr $，$ n(\vec{r}) $乃局部折射率，由此可推二介质反射折射之律。
% 费马原理者，光于两点之迹乃微程之迹，可谓之$ \int_{A}^{B} n(\vec{r}) dr $，$ n(\vec{r}) $乃局部折射率，由此可推二介质反射折射之律。
% 在其最普遍的形式中，费马原理可以表述为固定相原理\cite{feynman2010quantum}，也就是说，相对于路径的无穷小变化，沿着实际光路累积的相位导数$ \int_{A}^{B} d \varphi (\vec{r}) $将为零。
% 于其大体之中，费马理可立为定相理\cite{feynman2010quantum}，亦此之谓，于道之无小变，循实光路积之位导数$ \int_{A}^{B} d \varphi (\vec{r}) $将为零。
...
最近有一些科学研究，对两个介质的界面做一定的设计，可于光中引入相移之变$ \Phi (\vec{r}_\mathrm{s}) $，相移$ \Phi (\vec{r}_\mathrm{s}) $由沿界面之坐标$ \vec{r}_\mathrm{s} $所定。
...
% \autoref{fig:Snell-Law}中的入射平面波的入射角为$\theta_1$，假设两条光路无限接近实际光路，那么它们之间的相位差为零，有：
...
\end{equation}
此时$\theta_2$是反射角，入射角$\theta_1$和反射角$\theta_2$之间并不相等，这和用一面镜子做反射大不相同。
...
反射面的调控单元是周期性排列的，通过设定单元的反射矩阵可以实现对波束的随心调控。
下面我们通过一个具体的例子来解释这一问题。
\autoref{fig:phase}中有一个馈源$F$，它向四面八方发射球面波，通过RIS后，电磁波被调控为平面波。
反射波的方向垂直于RIS阵面。
...
值得说明的是，反射波束的方向是随心控制的，可以根据需求相应地调整。
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
根据上式，只要确定了$(\theta_0,\phi_0)$和相位延迟$F(m,n)$，就能得到RIS表面的$S(m,n)$，即相位延迟。
然后，我们在通过控制表面单元的电压或者表面元件的通断，去调节每个RIS单元的反射相位，达到随心调控电磁波的目的。
% 之后，只要将超表面阵列中各个结构单元的反射相位关系与超表面结构参数意义对应。
...
\autoref{fig:4-Bit_SIPO_Shift_Register}是值得学习的移位寄存器的接法，因为它采用了串进并出的先进架构。
打一个生动的比方：这就像老师领着一队小孩去食堂就餐，如果只有一个窗口，他们只能一个一个排队盛饭。
而如果食堂有很多窗口，所以虽然他们拍成一长队来食堂，但是几个小孩却可以同时盛饭，这大大提高了食堂的运转效率，减少了小孩们的等待时间。
% 在这种配置中，每个触发器是边沿触发。
% 所有触发器都以给定的时钟频率运行。
% 每个输入位在$n$个时钟周期后将移动到第$n$个触发器的输出端，导致并行输出。
如果食堂有$n$个窗口，则$n$个小孩可以同时盛饭，代表着$n$位的移位寄存器可以实现$n$位的数据并行输出。
...
在移位的过程中，我们不希望输出的数据随着移位的过程而改变，所以需要锁存器来对输出进行缓冲，如\autoref{fig:Logic-diagram}所示。
...
\autoref{fig:2shift}为典型的二级74LVC595ABQ级联电路示意图，两级级联之后，电路的性能直接翻倍，由原来的一个IO控制8个单元变成了$2 \times 8 = 16$个单元。
...
AXI总线协议由安谋控股公司提出，该公司后来被软银集团收购。
赛灵思从对AXI总线提供支持，这是一件令人欣喜的事情。
...
所选用的某一级代理商的核心板有1 GB DDR3 SDRAM，4 GB eMMC，资源丰富。
...
随后，基于上述的理论分析，设计了数款由IRS及其Control Circuit构成的RIS系统。
从IRS的选择入手，依次介绍了控制电路的设计和驱动、固件的设计。
